...
class Usuario:
    ...
    def propoe_lance(self, leilao, valor):
        if not self._valor_eh_valido(valor):
            raise LanceInvalido("Não é possível propor um lance com valor maior que o da carteira.")
...
class Leilao:
    ...
    def _valor_crescente_lance(self, lance):
    ...
    def _lance_eh_valido(self, lance):
        return not self._tem_lances() or (self._usuarios_diferentes(lance) and 
                                          self._valor_crescente_lance(lance))